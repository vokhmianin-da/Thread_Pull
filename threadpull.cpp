...
void ThreadPull::worker_thread(ThreadPull *pull)
{
    while(!pull->done)
    {
        ...
        {
            QString str = pull->codec->toUnicode("¬ычисл€етс€ факториал числа %1").arg(task.getVal());
            emit pull->MessageSender.SendMessage(str);
            task();
            str = pull->codec->toUnicode("‘акториал числа %1 равен %2").arg(task.getVal()).arg(task.getResult());
            emit pull->MessageSender.SendMessage(str);
        }
        else
        {
//            std::this_thread::yield();
            std::unique_lock<std::mutex> lk(pull->mut); //в отличие от std::lock_guard позвол€ет ожидающему потоку захватить управление мьютексом
...
void ThreadPull::submit(Task f)
{
    work_queue.push(f);
    IsWorked.notify_all();
    QString str = codec->toUnicode("ƒобавлена задача вычислени€ факториала числа %1").arg(f.getVal());
...
void ThreadPull::StartThreads()
{
    ...
    try
    {
        ...
        QString str = codec->toUnicode("«апущено %1 потоков пула").arg(thread_count);
        emit MessageSender.SendMessage(str);
    }
    catch(...)
    {
...
void ThreadPull::StopThreads()
{
    ...
    threads.clear();
    work_queue.clear(); //по заданию нужно удалить оставшиес€ в очереди задачи
    QString str = codec->toUnicode("ѕул потоков остановлен");
    emit MessageSender.SendMessage(str);
}
